...
class BoundingBox:
    def __init__(self, x0: int, y0: int, x1: int, y1: int, label="Новая метка", color=Qt.GlobalColor.green):
        """
    ...
    def __str__(self):
        return f"Метка {self.label} P0: {self.p0} P1: {self.p1}"
    ...
    def copy(self):
        return BoundingBox(*self.p0, *self.p1, label=self.label)
